...
- Add `analytics` skill with a repo-agnostic base and a MetaMask Mobile overlay for the canonical tracking API. Marked `mandatory: true` so it installs even when the `coding` domain is filtered out.
